...
**Résultats.** `CrossCuttingConfigProcessor` → dispatcher ~58 lignes sur `List<CrossCuttingRewriter>` (8 rewriters, voir [[cross-cutting-config-pattern]]). `ResourceExpandProcessor` → orchestration sur `List<DbVariant>`/`List<IdVariant>` (voir [[resource-expand-db-variants]]). PMD 71 → 45, SpotBugs `REC_CATCH_EXCEPTION` résolu, JaCoCo 96.6 % instr / 82.2 % branches, 143 → 158 tests, Checkstyle 0. Résidu assumé : complexité de `RealmRewriter` (boucles imbriquées) et `TestAllRewriter` (émetteur de script, `@SuppressWarnings` justifié).

**Commits :** `1cbb9ed` (golden-master) → `f45345e` (plan) → `7237e04` style → `836a365` ResourceNaming → `96aa4dd` YamlBlocks → `5177ab4` dispatcher+8 rewriters → `d947761` DbVariant/IdVariant → `22416e4` catch IOException.
