...
**Architecture (2026-07-06 refactor): the `switch`-on-enum was replaced by strategy beans.**
`ResourceExpandProcessor` (`@Order(50)`) is now orchestration only: it injects `List<DbVariant>` + `List<IdVariant>` and dispatches by linear scan on `type()` (N≤3, no `EnumMap`). Each strategy is a `@Component` implementing one interface with a `type()` discriminant: `DbVariant` → `PostgresVariant` (no-op), `H2Variant`, `MongoVariant`; `IdVariant` → `LongIdVariant` (no-op), `IntegerIdVariant`, `UuidIdVariant`. The old `applyMongo` (PMD NPath 432) was decomposed into named steps (`applyJava`/`applicationYml`/`pom`/`entity`/`repository`/`otherJava`) inside `MongoVariant`, collapsing the complexity. Adding a `DatabaseType`/`IdType` now = one new strategy bean (SOLID open/closed), not a new `switch` arm. This killed `GodClass`/`TooManyMethods`/`NPath` on the processor. The per-file propagation gotchas below are unchanged — they now live inside `MongoVariant`/`UuidIdVariant`. The strategies live in package `pipeline.processor` (share package-private `ProcessorUtils`/`ResourceNaming`). Golden-master (7 cases) + the 28 existing `ResourceExpandProcessorTest` tests are the non-regression oracle — no separate per-variant unit tests were added (behavior-constant refactor). Those 28 tests were later split by variant into `ResourceExpandProcessorTest` (core) / `ResourceExpandH2Test` / `ResourceExpandMongoTest` / `ResourceExpandIdTypeTest`, with the big `service-a` fixture extracted to `ResourceExpandFixtures` (to clear PMD `TooManyMethods`).

`ResourceExpandProcessor` derives one business service per `resources[]` entry by cloning the `service-a/` template, then applies `databaseType` and `idType` variants. The trap: each generated file is transformed independently, so a change to the **entity's package or id type must be propagated to every other file that references it**. Three separate breaks this session all came from forgetting that propagation — each surfaced only at compile/runtime, never in unit tests, because the test fixture for the service layer was a trivial stub that didn't import the entity.
