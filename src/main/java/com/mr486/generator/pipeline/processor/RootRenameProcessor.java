 ...
 *
 * <p>Si les deux préfixes sont identiques (requête sans {@code name}), le processor est un no-op et
...
public class RootRenameProcessor implements FileProcessor {
    ...
    public List<GeneratedFile> process(List<GeneratedFile> files, GenerationContext ctx) {
        if (ctx.getSourceRoot().equals(ctx.getTargetRoot())) {
            return files;
        }
        ...
        return files.stream()
            .map(f -> f.path().startsWith(prefix)
                ? new GeneratedFile(
                    ctx.getTargetRoot() + f.path().substring(prefix.length() - 1),
                    f.content(), f.executable())
                : f)
            .toList();
    }
}
