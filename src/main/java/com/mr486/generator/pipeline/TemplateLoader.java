 ...
 * <p>
 * Le modèle est stocké décompressé (un fichier = une entrée), ce qui rend ses diffs Git lisibles.
 ...
 * <p>
 * Les répertoires sont ignorés et le bit exécutable Unix est dérivé heuristiquement de l'extension
 * {@code .sh} ou du nom {@code mvnw} (les permissions ne survivent pas au classpath).
 * <p>
 * <b>Convention dotfiles</b> : le modèle ne contient aucun vrai fichier caché. Les dotfiles sont
...
public class TemplateLoader {
    ...
    private static List<GeneratedFile> loadFromClasspath() {
        ...
        try {
            ...
            for (Resource r : resources) {
                if (!r.isReadable()) continue;                 // écarte les répertoires (filesystem)
                String url = r.getURL().toString();
                int idx = url.indexOf(TEMPLATE_PREFIX);
                if (idx < 0) continue;
                String path = url.substring(idx + "templates/".length());  // -> "ms-platform/..."
                if (path.isEmpty() || path.endsWith("/")) continue;        // écarte les répertoires (jar)
    ...
    private static String decodeDotfile(String path) {
        ...
        if (name.startsWith("dot-")) name = "." + name.substring("dot-".length());
        return dir + name;
    }
}
